...
void set_b1_priors_mpp(double b1_min, double b1_max)
{
   printf("priors_mpp.c: set_b1_priors_mpp: flat bias priors should be applied in python script\n");
}

void set_b2_priors_mpp(double b2_min, double b2_max)
{
   printf("priors_mpp.c: set_b2_priors_mp: flat bias priors should be applied in python script\n");
}

void set_ia_priors_mpp(double A_min, double A_max)
{
   printf("priors_mpp.c: set_ia_priors_mpp: flat A priors should be applied in python script\n");
}
...
void set_shear_priors_mpp(double *mean_m,double *sigma_m)
{
  ...
  for (int i=0;i<10; i++){
    ...
    if (sigma_m[i]){
      printf("zn = %d (mean,sigma) = (%e, %e)\n",i, prior.shear_calibration_m[i][0],prior.shear_calibration_m[i][1]);
      like.shearcalib=1;
    } 
  }
  printf("\n");
...
void set_wlphotoz_priors_mpp(double *bias_photoz_s,double *sigma_b_photoz_s){
  ...
  for (int i=0;i<10; i++){
    ...
    if (sigma_b_photoz_s[i]){
      printf("zn = %d (mean,sigma) = (%e, %e)\n",i, prior.bias_zphot_shear[i][0],prior.bias_zphot_shear[i][1]);
      like.wlphotoz=1;
    }
  }
  printf("\n");
