    ...
    for( nl1 = 0; nl1 < Cluster.lbin; nl1 ++){
      for (nN2 = 0; nN2 < Cluster.N200_Nbin; nN2 ++){
        for( nl2 = 0; nl2 < Cluster.lbin; nl2 ++){
          ...
          if (nl2 == nl1){c_g =cov_G_cgl_cgl(ell_Cluster[nl1],nzc1,nN1, nzs1, nzc2, nN2,nzs2)/((2.*ell_Cluster[nl1]+1.)*dell_Cluster[nl1]*fsky);}
  ...
  for (nN2 = 0; nN2 < Cluster.N200_Nbin; nN2 ++){
    for( nl2 = 0; nl2 < Cluster.lbin; nl2 ++){
      ...
      if (ell[nl1] < like.lmax_shear){
        ...
        if (fabs(ell[nl1]/ell_Cluster[nl2] -1.) < 0.1){ c_g =cov_G_shear_cgl(ell[nl1],nzs1,nzs2, nzc2, nN2,nzs3)/((2.*ell[nl1]+1.)*dell[nl1]*fsky);}
      }
  ...
  for (nN2 = 0; nN2 < Cluster.N200_Nbin; nN2 ++){
    for( nl2 = 0; nl2 < Cluster.lbin; nl2 ++){
      ...
      if (weight){
        ...
        if (fabs(ell[nl1]/ell_Cluster[nl2] -1.) < 0.1){c_g =cov_G_ggl_cgl(ell[nl1],zl,zs, nzc2, nN2,nzs3)/((2.*ell[nl1]+1.)*dell[nl1]*fsky);}
      }
    ...
    for (nN2 = 0; nN2 < Cluster.N200_Nbin; nN2 ++){
      for( nl2 = 0; nl2 < Cluster.lbin; nl2 ++){
        ...
        if (weight){
          ...
          if (fabs(ell[nl1]/ell_Cluster[nl2] -1.) < 0.1){c_g =cov_G_cl_cgl(ell[nl1],N1,N1, nzc2, nN2,nzs3)/((2.*ell[nl1]+1.)*dell[nl1]*fsky);}
        }
  ...
  for (nl1 = 0; nl1 < like.Ncl; nl1 ++){
    for (nl2 = 0; nl2 <like.Ncl; nl2 ++){
      ...
      if (weight && ell[nl2] < like.lmax_shear){
        ...
        if (nl1 == nl2){
          c_g =  cov_G_gl_shear_tomo(ell[nl1],zl,zs,z3,z4)/((2.*ell[nl1]+1.)*dell[nl1]*fsky);
        }
      }
...
void run_cov_clustering_shear(char *OUTFILE, char *PATH, double *ell, double *dell, int n1, int n2,int start)
{
  ...
  for (nl1 = 0; nl1 < like.Ncl; nl1 ++){
    for (nl2 = 0; nl2 <like.Ncl; nl2 ++){
      ...
      if (nl1 == nl2){
        c_g =  cov_G_cl_shear_tomo(ell[nl1],z1,z2,z3,z4)/((2.*ell[nl1]+1.)*dell[nl1]*fsky);
      }
    }
    ...
    for (nl2 = 0; nl2 < like.Ncl; nl2 ++){
      ...
      if (z1 == zl){
        ...
        if (weight){
          ...
          if (nl1 == nl2){
            c_g =  cov_G_cl_gl_tomo(ell[nl1],z1,z2,zl,zs)/((2.*ell[nl1]+1.)*dell[nl1]*fsky);
          }
        }
      }
  ...
  for (nl1 = 0; nl1 < like.Ncl; nl1 ++){
    for (nl2 = 0; nl2 < like.Ncl; nl2 ++){
      ...
      if (z1 == z3){
        ...
        if (nl1 == nl2){
          c_g =  cov_G_cl_cl_tomo(ell[nl1],z1,z2,z3,z4)/((2.*ell[nl1]+1.)*dell[nl1]*fsky);
        }
      }
...
void run_cov_ggl(char *OUTFILE, char *PATH, double *ell, double *dell, int n1, int n2,int start)
{
  ...
  for (nl1 = 0; nl1 < like.Ncl; nl1 ++){
    for (nl2 = 0; nl2 < like.Ncl; nl2 ++){
      ...
      if (nl1 == nl2){
        c_g =  cov_G_gl_gl_tomo(ell[nl1],zl1,zs1,zl2,zs2)/((2.*ell[nl1]+1.)*dell[nl1]*fsky);
      }
      if (weight ==0 && n2 != n1){
        c_g = 0;
      }
...
void run_cov_shear_shear(char *OUTFILE, char *PATH, double *ell, double *dell,int n1, int n2,int start)
{
  ...
  for (nl1 = 0; nl1 < like.Ncl; nl1 ++){
    for (nl2 = 0; nl2 < like.Ncl; nl2 ++){
      ...
      if (nl1 == nl2){
        c_g =  cov_G_shear_shear_tomo(ell[nl1],z1,z2,z3,z4)/((2.*ell[nl1]+1.)*dell[nl1]*fsky);
 ...
 int main(int argc, char** argv)
 {
   ...
   init_binning(10.);
   init_survey("HSC");
   init_galaxies("../zdistris/zdistribution_HSC","../zdistris/zdistribution_const_comoving", "none", "gaussian", "redmagic");
   init_clusters();
   init_IA("none", "GAMA");
