    ...
    for (i = 0; i< tomo.shear_Nbin; i++){
      ...
      switch (redshift.shear_photoz){
        case 0: // no photo-zs, split 'true' n(z) histogram in bins
          ...
          for (k = 0;k<zbins; k++){
            table[i+1][k] = 0;
            if (z_v[k] >= array[0] && z_v[k] <= array[1]){ table[i+1][k] = zdistr_histo_1(z_v[k],(void*)array)/norm;}
          }
          break;
        case 1: //Pseudo Voigt (Lorentzian + Gaussian) photo-zs
          ...
          for (k = 0;k<zbins; k++){
            x1 =(array[0] -z_v[k] + bias_zphot_shear(z_v[k],i))/(sqrt(2.)*sigma_zphot_shear(z_v[k],i));
            x2 = (array[1]-z_v[k] + bias_zphot_shear(z_v[k],i))/(sqrt(2.)*sigma_zphot_shear(z_v[k],i));
            table[i+1][k] = zdistr_histo_1(z_v[k],(void*)array)*(eta*0.3183*(atan(x2)-atan(x1)) + ((1.0-eta)*0.5*(gsl_sf_erf(x2)-gsl_sf_erf(x1))));
      ...
      // I think the above implementation is not correctly normalized for the outliers. Instead do it at the normalization step. This should probably be updated to allow for a leakage matrix.
        ...
        case 3: //Gaussian photo-zs
          ...
          for (k = 0;k<zbins; k++){
            x1 =(array[0] -z_v[k] + bias_zphot_shear(z_v[k],i))/(sqrt(2.)*sigma_zphot_shear(z_v[k],i));
            x2 = (array[1]-z_v[k] + bias_zphot_shear(z_v[k],i))/(sqrt(2.)*sigma_zphot_shear(z_v[k],i));
            table[i+1][k] = 0.5*zdistr_histo_1(z_v[k],(void*)array)*(gsl_sf_erf(x2)-gsl_sf_erf(x1));
            norm += table[i+1][k]*da;
          }
          for (k = 0;k<zbins; k++){table[i+1][k]/= norm;}
          break;
